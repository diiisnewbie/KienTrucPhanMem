...
// Xử lý nghiệp vụ, validate dữ liệu, kích hoạt hook
// Chỉ giao tiếp với Data Access (tầng dưới) và Presentation (tầng trên)

const dal = require('../dataAccess/dataAccessLayer');

// Lưu trữ hook callbacks trong memory (runtime)
const hookRegistry = {};
...
async function updateContent(id, title, body, status) {
  await getContent(id); // kiểm tra tồn tại
...
async function activatePlugin(name) {
  ...
  // Tự động đăng ký hook mẫu khi activate
